...
def case_1(binary_array, bottom, top):  # Receive the first and last index of array as top and bottom
...
def case_2(binary_array, bit_location):
    ...
    if len(lower) == (math.pow(2, bit_location - 1)):
        return "1" + case_2(higher, bit_location - 1)
    else:
        return "0" + case_2(lower, bit_location - 1,)
...
if __name__ == "__main__":
    ...
    # 2) Case of unsorted
    random.shuffle(binary_array)  # Randomize the array.
    most_significant_bit = math.ceil(math.log(array_length + 1, 2))

    missing_number_binary = case_2(binary_array, most_significant_bit)
    missing_number = int(missing_number_binary, 2)
    print(missing_number)
